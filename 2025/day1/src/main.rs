...
fn main(){
    ...
    let contents_raw: String = fs::read_to_string(&args[1])
    ...
    for (_index, line) in contents.into_iter().enumerate()
    {
        let temp = current + translate_move(line);
        current = if temp < 0 { 100 - temp.abs() } else if temp >= 100 { temp - 100 } else { temp };
        // println!("{index}: move: {line} current after move: {}", current);
        if current == 0 
        {
            zero_hits += 1;
        }
...
fn translate_move(a_move: &str) -> i32 {
    let distance = a_move[1..].trim().parse::<i32>()
